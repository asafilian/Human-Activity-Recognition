---
title: "A Predictor for Human Activity Recognition"
author: "Ali Safilian"
...
In [Sect. 1](#sec-data), we split the data into training and testing datasets.  
...
```
 ...
 Two datasets are available, `pml-training.csv` and `pml-testing.csv`. We use the former for training, and save it as `dataset`. The latter is used for validation of the model, and we load it to a variable called `validation`. In addition, we split `dataset` to two sets, `train.set` and `test.set`. We build our models on `train.set` and test whether it overfits or not on `test.set`.
...
# Preprocessing
In this section, we do preprocessing on `train.set`. We save the preprocessed data into a variable called `train.set.proc`. 
...
Now, we are going to remove the varaibles that have more than 50% missing values. Let us first see which variables have missing values. In the following script, we save extract all variables that have NA values and save their indices in the `ids_NA.1` varible. 
...
sum(colSums(is.na(train.set.proc)))
dim(train.set.proc)
```

Now, there is no NA values in any columns of the training data set, we got `r  dim(train.set.proc)[2]` variables.
...
Let us extract the variables with near zero variability:
...
This is a good mixture of simple linear (LDA), nonlinear (CART, kNN) and complex nonlinear methods (SVM, RF). We reset the random number seed before reach run to ensure that the evaluation of each algorithm is performed using exactly the same data splits. 
...
We can see that the most accurate models in this case were random forests and KNN. The former is a little bit more accurate than KNN. Let us take a look at the randome forests model:

```{r rf-summary}
print(fit.rf)
```

Although, random forests provide use the most accuracte model. However, it may over-fit. To see if this is the case, we are going to predict it on our testing set `test.set`. We first need to preprocess `test.set` based on the preprocessing we got for training set. 
...
As we see, we get about 98% accuracy, which shows that our random forests model work pretty well. This well represented in the following plot.
